...
function SendMessage() {
    let msgVar = document.getElementById('txtMessage').value;
    if (msgVar.length != 0) {
        var chatMessage = {
            userId: currentUserKey,
            msg: msgVar,
...
function LoadChatList() {
    ...
    db.on('value', function (lists) {
        document.getElementById('lstChat').innerHTML = `<li class="list-group-item" style="background-color:#3C3B5C;">
                            <input style="background-color: #3C3B5C; color: white;" type="text" placeholder="Search or new chat" class="form-control form-rounded" />
                        </li>`;
        lists.forEach(function (data) {
            ...
            if (friendKey !== "") {
                firebase.database().ref('users').child(friendKey).on('value', function (data) {
                    var user = data.val();
                    document.getElementById('lstChat').innerHTML += `<li class="list-group-item list-group-item-action" onclick="StartChat('${data.key}', '${user.name}', '${user.photoURL}')" style="background-color: #3C3B5C !important;">
                            <div class="row">
                                <div class="col-md-2">
...
function PopulateUserList() {
    ...
    db.on('value', function (users) {
        if (users.hasChildren()) {
            lst = `<li class="list-group-item" style="background-color:#3C3B5C !important;">
                            <input type="text" style="background-color: #3C3B5C; color: white;" placeholder="Search or new chat" class="form-control form-rounded" />
                        </li>`;
            ...
            if (user.email !== firebase.auth().currentUser.email) {
                dbNoti.orderByChild('sendTo').equalTo(data.key).on('value', function (noti) {
                    if (noti.numChildren() > 0 && Object.values(noti.val())[0].sendFrom === currentUserKey) {
                        lst = `<li class="list-group-item list-group-item-action bgDBlue">
                            <div class="row">
                                <div class="col-md-2">
                        ...
                        dbNoti.orderByChild('sendFrom').equalTo(data.key).on('value', function (noti) {
                            ...
                            <div class="row">
                                ...
                                lst = `<li class="list-group-item list-group-item-action bgDBlue" data-dismiss="modal">
                            <div class="row bgDBlue">
                                <div class="col-md-2">
                                    <img src="${user.photoURL}" class="rounded-circle friend-pic" />
                                </div>
                                <div class="col-md-10" style="cursor:pointer;">
...
function PopulateNotifications() {
    ...
    db.orderByChild('sendTo').equalTo(currentUserKey).on('value', function (notis) {
        if (notis.hasChildren()) {
            lst = `<li class="list-group-item" style="background-color:#3C3B5C;">
                            <input type="text" style="background-color: #3C3B5C; color: white;" placeholder="Search or new chat" class="form-control form-rounded" />
                        </li>`;
        }
        notis.forEach(function (data) {
            ...
            if (noti.status === 'Pending') {
                lst += `<li class="list-group-item list-group-item-action" style="background-color: #3C3B5C;">
                            <div class="row">
                                <div class="col-md-2">
                                    <img src="${noti.photo}" class="rounded-circle friend-pic" />
                                </div>
                                <div class="col-md-10" style="cursor:pointer; display: flex; justify-content: center; align-items: center;">
